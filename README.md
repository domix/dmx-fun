...
| `fun-spring-webflux` | [![Maven Central](https://img.shields.io/maven-central/v/codes.domix/fun-spring-webflux)](https://central.sonatype.com/artifact/codes.domix/fun-spring-webflux) | Spring WebFlux adapters — map `Option`, `Result`, `Try`, and `Validated` to `ServerResponse` in functional endpoints, with `Flux` aggregation, custom success status/headers, and RFC 7807 `ProblemDetail` responses. Builds on `fun-reactor`. |
