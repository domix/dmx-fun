...
| `fun-spring-webflux` | [![Maven Central](https://img.shields.io/maven-central/v/codes.domix/fun-spring-webflux)](https://central.sonatype.com/artifact/codes.domix/fun-spring-webflux) | Spring WebFlux adapters — map `Option`, `Result`, `Try`, and `Validated` to `ServerResponse` in functional endpoints (`WebfluxFun`) or `ResponseEntity` in annotation controllers (`WebfluxEntity`), with `Flux` aggregation, custom success status/headers, and RFC 7807 `ProblemDetail` responses. Builds on `fun-reactor`. |
...
| `fun-reactor`      | [![Maven Central](https://img.shields.io/maven-central/v/codes.domix/fun-reactor)](https://central.sonatype.com/artifact/codes.domix/fun-reactor)           | Project Reactor interop — convert `Option`, `Result`, `Try` to/from `Mono`/`Flux`, plus railway operators and `sequence`/`collectValidated` aggregation. |
...
Map `Option`, `Result`, `Try`, and `Validated` to a `ServerResponse` in functional endpoints via `WebfluxFun`, or to a `ResponseEntity` in annotation controllers via `WebfluxEntity`, with `Flux` aggregation. Builds on `fun-reactor`. `spring-webflux` is declared as `compileOnly`; bring your own version (tested: 6.0.x – 7.0.x).
