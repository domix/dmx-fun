...
import {Content as SpringBootAuto}  from '../code/guide/jackson/spring-boot-auto.mdx';
...
| `Try<V>`             | `v` (unwrapped)   | `{"error": "msg"}` or `{"error": null}` |
...
### Auto-configuration with `fun-spring-boot`

If you use `fun-spring-boot` (instead of wiring things manually), `DmxFunModule` is
registered as a Spring bean automatically whenever both `jackson-databind` and
`fun-jackson` are on the classpath. Spring Boot's `JacksonAutoConfiguration`
discovers all `Module` beans and applies them to the `ObjectMapper` — no
`@Bean DmxFunModule` declaration required.

<SpringBootAuto/>

The auto-configuration follows standard Spring Boot conventions:

| Condition | Behaviour |
|---|---|
| `jackson-databind` **and** `fun-jackson` on classpath | `DmxFunModule` bean registered |
| User declares own `DmxFunModule` `@Bean` | Auto-configuration backs off (`@ConditionalOnMissingBean`) |
| `dmx.fun.jackson.enabled=false` | Auto-configuration disabled entirely |
...
`Try.failure(ex)` serializes as `{"error": "msg"}` where `msg` is
`ex.getMessage()`. If the exception has no message, the field is written as
`{"error": null}` and a `RuntimeException` with a `null` message is
reconstructed on deserialization. In all cases only the message survives —
the original exception class and stack trace are not preserved.
