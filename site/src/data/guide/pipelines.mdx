...
| `spring-boot-compatibility.yaml`         | PR touching `spring-boot/**` or `spring/**`        | Matrix test of `fun-spring-boot` against Spring Boot 3.3.x – 4.1.x       |
| `spring-boot-sample-compatibility.yaml`  | PR touching `spring-boot-sample/**` or `spring/**` | Matrix test of the Spring Boot sample app against Boot 3.4.x – 4.1.x     |
...
**Local equivalent**:
```bash
./gradlew :spring-boot:test -PspringBootVersion=4.1.0
```
...
3. Runs `pnpm run build` inside `site/`
