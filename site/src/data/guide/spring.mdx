...
`DmxFunModule` is the Jackson serializer/deserializer for all dmx-fun types. When it is registered as a bean, Spring Boot's `JacksonAutoConfiguration` applies it to the `ObjectMapper` automatically — `Result`, `Option`, `Try`, and the other types serialize to JSON without any extra wiring. See the [Jackson Integration guide](./jackson#auto-configuration-with-fun-spring-boot) for the full reference.
...
Both the 3.x and 4.x release lines are supported. Tested in CI against the
