...
|---|---|
| `TxResult` | Always (spring-tx present) |
| `TxTry` | Always (spring-tx present) |
| `TxValidated` | Always (spring-tx present) |
| `DmxTransactionalAspect` | `aspectjweaver` is also on the classpath |
...
Every bean that `DmxFunSpringAutoConfiguration` registers is guarded by
`@ConditionalOnMissingBean`. Declare your own `@Bean` of the same type and the
auto-configuration backs off, leaving yours in place.

The most common reason to override is to bind a specific transaction manager rather
than the primary one:

<SpringBootOverride/>

You can override any subset — any bean not overridden is still registered by the
auto-configuration.
...
Tested in CI against the following versions on every pull request that touches
the `spring-boot/` or `spring/` module:
...
```bash
./gradlew :spring-boot:test -PspringBootVersion=4.0.5
```
