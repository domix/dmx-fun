...
Commit with:
```bash
git commit -m "chore(release): bump version to 0.1.0" gradle.properties
...
1. Detects that `version=0.1.0` is **not** a SNAPSHOT
2. Checks that the git tag `v0.1.0` does not already exist
3. Runs `./gradlew test` — all modules
4. Runs `./gradlew publishToMavenCentral --no-configuration-cache`
5. Creates and pushes the annotated tag `v0.1.0`
...
git commit -m "chore(release): bump version to 0.1.1-SNAPSHOT" gradle.properties
git push origin main
```
...
Tags follow the pattern `v{version}` — e.g. `v0.1.0`.
...
create the release manually via `gh release create v0.1.0 --generate-notes`.
