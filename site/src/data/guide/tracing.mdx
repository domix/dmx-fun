...
`fun-resilience4j`. Any version from **1.2.x through 1.7.x** is supported
...
> **Warning:** The default `exception` tag uses `getClass().getSimpleName()`. When
> arbitrary third-party exceptions can appear at runtime, this produces an unbounded number
> of distinct tag values. High-cardinality span tags generate excessive index entries in
> Zipkin, Jaeger, and similar backends — and bloat sampling metadata in OpenTelemetry exporters.
